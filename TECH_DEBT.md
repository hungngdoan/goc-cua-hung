...
- Known tradeoff: the check restates the twelve sections itself (`check-section-registry.mjs:30`) rather than importing the registry, so adding a tab touches two files rather than one. That is deliberate. A golden check that imports the thing it checks would pass no matter what the registry said.
